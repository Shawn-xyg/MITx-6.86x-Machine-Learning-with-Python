...
def epsilon_greedy(state_vector, epsilon):
    ...
    """
    # TODO Your code here
    rand_C = np.random.randint(0, NUM_ACTIONS), np.random.randint(0, NUM_OBJECTS)
    act_arr, obj_arr = model(state_vector)
    maxQ = (torch.argmax(act_arr), torch.argmax(obj_arr))
    res = np.random.choice(a = [0, 1], p = [epsilon, 1-epsilon])
    action_index, object_index = rand_C if res == 0 else maxQ
